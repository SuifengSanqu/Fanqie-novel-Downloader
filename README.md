...
已修复超过 500 本时后台拒绝、界面却不显示错误的问题。当前版本每次支持去重后最多 10,000 本，
一个任务按清单顺序逐本下载，不会同时启动数千本；退出后可在“历史”继续未完成任务。
解析结果会显示数量与上限；超限、目录无效或任务冲突等错误直接显示在导入弹窗中，输入不会丢失。
若连接中断、未确认创建成功，请先检查任务列表，再决定是否重试。
