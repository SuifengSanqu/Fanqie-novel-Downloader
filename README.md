...
桌面端与移动端共用同一套阅读器界面。下面的图片来自当前 Windows x64 测试版，使用隔离的
应用数据目录生成，不包含个人书架、账号、Cookie 或本地路径信息。

![应用主界面](docs/images/app-overview.png)

![阅读器正文与章节信息](docs/images/reader-chapter.png)

自动阅读面板只保留速度和开始操作，启动后会立即收起；运行时底栏入口显示启用状态，点击入口
即可暂停。

![自动阅读面板](docs/images/reader-auto-reading.png)

![自动阅读运行状态](docs/images/reader-auto-running.png)

阅读器还支持章节目录、字号/主题调整和可选的沉浸模式。目录展开或收起时会保持当前操作位置，
不会把长目录重新跳回当前章节。Android 与 iOS 的导出、分享和系统文件访问会根据平台能力
显示对应入口。
...
- 实时连续自动阅读：当前章节正文到底后直接无缝追加下一章，不跳页、不重置滚动位置；按下正文时暂停，手指或鼠标松开 3 秒后自动恢复
- 长目录展开或收起时保持操作节点的位置和焦点，不会跳回当前阅读章节
...
7. 阅读时可从底栏打开“自动阅读”，选择慢、标准或快三档速度后点击“开始”；自动阅读会实时
   连续下滑并在章节末尾直接接续下一章。阅读中按住正文会暂停，松开手指或鼠标后等待 3 秒
   自动继续；点击底栏入口则永久暂停。
...
不需要。设置页的开关、选项和目录选择会立即自动保存；文件名格式在停止输入 0.5 秒或离开输入框后保存。
顶部会显示等待保存、保存中、已保存或失败状态，退出前请确认已保存。无效的文件名格式不会提交，需先修正；
保存失败会保留当前输入，可点“重试保存”。本机 WebView / 浏览器存储可用时，快速关闭或刷新后还会恢复待保存草稿并再次尝试保存。
启动页面在自动保存后下次启动生效；连续修改和后台状态刷新不会覆盖待保存的编辑。
